...
export const DriverEarningsModal: React.FC<DriverEarningsModalProps> = ({ isOpen, onClose, tripHistory, initialTab = 'EARNINGS' }) => {
  ...
  // Create a copy before sorting to avoid mutation of props
        ...
        {driverHistory.map((trip) => (
          <div key={trip.id} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
             ...
             <div className="text-xs space-y-1 pl-2 border-l-2 border-gray-100">
                <p className="text-gray-500 truncate"><span className="w-2 h-2 inline-block bg-green-500 rounded-full mr-1"></span> {trip.pickupAddress}</p>
                <p className="font-medium truncate"><span className="w-2 h-2 inline-block bg-black rounded-full mr-1"></span> {trip.destinationAddress}</p>
             </div>
    ...
    return (
      <div className="space-y-6 animate-fade-in">
        ...
        <div className="bg-white p-5 rounded-2xl border border-gray-200 shadow-sm">
           <h3 className="font-bold text-sm mb-4">Horas Conectado (Semana Actual)</h3>
           ...
           <div className="mt-4 text-center">
              <span className="text-xs font-bold bg-gray-100 px-3 py-1 rounded-full">Total Semanal: {stats.weekHours}h</span>
           </div>
        </div>
      </div>
    );
