...
import { 
  ...
  CheckCircle2, MessageSquare, LogOut, Lock 
} from 'lucide-react';
// ... imports
...
export default function App() {
  ...
  const handleRegister = async (user: Omit<User, 'id' | 'status'>, pass: string, coopCode: string) => {
     try {
        ...
        setAuthSuccess("Registro exitoso. Tu cuenta esta PENDIENTE DE APROBACIÓN.");
     } catch (e: any) {
        setAuthError(e.message);
     }
  };
  ...
  const handleAddressInput = async (val: string, field: typeof activeField) => {
  ...
  const renderSavedPlacesSuggestions = (type: 'PICKUP' | 'DESTINATION' | 'SCHEDULE_PICKUP' | 'SCHEDULE_DESTINATION') => {
     return (
        <div className="absolute top-full left-0 right-0 bg-white dark:bg-gray-800 rounded-b-xl shadow-xl z-[70] overflow-hidden border-t border-gray-100 dark:border-gray-700 animate-slide-up max-h-[50vh] overflow-y-auto">
           <button onClick={() => { setIsPickingOnMap(true); setPickingTarget(type); if (type.includes('SCHEDULE')) setIsSchedulingOpen(false); setActiveField(null); }} className="w-full flex items-center gap-3 p-3 hover:bg-gray-50 dark:hover:bg-gray-700 text-left transition border-b border-gray-50 dark:border-gray-700 bg-black text-white dark:bg-white dark:text-black">
              ...
              <div className="border-b border-gray-100 dark:border-gray-700">
                 ...
                 {addressSuggestions.map((place, idx) => (
                    <button key={`api-${idx}`} onClick={() => {
                          ...
                          setActiveField(null); setAddressSuggestions([]);
                       }} className="w-full flex items-center gap-3 p-3 hover:bg-gray-50 dark:hover:bg-gray-700 text-left transition">
                       <div className="p-2 bg-gray-100 dark:bg-gray-600 rounded-full text-gray-500 dark:text-gray-300"><MapPin size={16} /></div>
                       <div><p className="font-bold text-sm dark:text-white line-clamp-1">{place.address}</p>{place.subtitle && <p className="text-[10px] text-gray-500 line-clamp-1">{place.subtitle}</p>}</div>
                    </button>
                 ))}
              </div>
           )}
              ...
              <div>
                 ...
                 {currentUser.savedPlaces.map((place, idx) => (
                    <button key={idx} onClick={() => {
                          if (type === 'PICKUP') { setPickupText(place.address); setPickupLocation(place.coords); } 
                          else if (type === 'DESTINATION') { setDestinationText(place.address); setDestinationLocation(place.coords); }
                          else if (type === 'SCHEDULE_PICKUP') { setScheduleForm(prev => ({ ...prev, pickup: place.address, pickupCoords: place.coords })); }
                          else if (type === 'SCHEDULE_DESTINATION') { setScheduleForm(prev => ({ ...prev, destination: place.address, destinationCoords: place.coords })); }
                          setActiveField(null); setAddressSuggestions([]);
                       }} className="w-full flex items-center gap-3 p-3 hover:bg-gray-50 dark:hover:bg-gray-700 text-left transition">
                       <div className="p-2 bg-gray-100 dark:bg-gray-600 rounded-full">{place.type === 'HOME' ? <Home size={16} /> : place.type === 'WORK' ? <Briefcase size={16} /> : <Heart size={16}/>}</div>
                       <div><p className="font-bold text-sm dark:text-white">{place.name}</p><p className="text-xs text-gray-500 line-clamp-1">{place.address}</p></div>
                    </button>
                 ))}
              </div>
           )}
  ...
  return (
    <div className="h-full w-full relative flex flex-col font-sans bg-white dark:bg-gray-900 text-slate-900 dark:text-white overflow-hidden">
      ...
      <nav className="absolute top-0 left-0 right-0 p-4 z-40 flex justify-between items-center pointer-events-none">
        <button onClick={() => setIsProfileOpen(true)} className="bg-white dark:bg-gray-800 text-black dark:text-white p-3 rounded-full shadow-lg pointer-events-auto hover:scale-105 transition"><Menu size={24} /></button>
        <div className="pointer-events-auto flex items-center gap-2">
           <button onClick={toggleTheme} className="bg-white dark:bg-gray-800 p-3 rounded-full shadow-lg">{theme === 'dark' ? <Sun size={20} className="text-white"/> : <Moon size={20} />}</button>
           <button onClick={() => setIsNotificationsOpen(true)} className="bg-white dark:bg-gray-800 text-black dark:text-white p-1 pr-3 rounded-full shadow-lg flex items-center gap-2">
             ...
             {currentCooperative ? <span className="text-[9px] uppercase font-black bg-yellow-400 text-black px-1 rounded">{currentCooperative.code}</span> : <span className="text-[9px] uppercase font-black bg-gray-200 text-gray-600 px-1 rounded">GLOBAL</span>}
         ...
         <div className="absolute bottom-10 left-0 right-0 px-6 z-50 flex flex-col items-center animate-slide-up">
            <div className="bg-white dark:bg-gray-800 p-4 rounded-2xl shadow-2xl w-full max-w-md text-center space-y-4">
               ...
               <button onClick={async () => {
                 ...
                 }} className="w-full bg-black dark:bg-white text-white dark:text-black py-3 rounded-xl font-bold">Confirmar ubicación</button>
               <button onClick={() => { 
      ...
      {currentUser.role === UserRole.DRIVER && status === RideStatus.IDLE && !isPickingOnMap && (
        <div className="absolute bottom-0 w-full bg-white dark:bg-gray-900 rounded-t-3xl shadow-2xl z-30 p-6 animate-slide-up">
           <div className="flex justify-between items-center mb-6">
              <div><h2 className="text-2xl font-bold dark:text-white">Panel Conductor</h2><p className="text-gray-400 text-sm flex items-center gap-2">{isDriverOnline ? <>🟢 En línea <span className="font-mono text-black dark:text-white bg-gray-100 dark:bg-gray-800 px-2 rounded-md ml-1">{onlineDuration}</span></> : '🔴 Desconectado'}</p></div>
           ...
           <div className="bg-gray-50 dark:bg-gray-800 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 flex items-center gap-4 mb-4">
              <div className="w-16 h-12 bg-white dark:bg-gray-700 rounded-lg overflow-hidden shadow-sm flex items-center justify-center">{currentUser.driverDetails?.carPhotoUrl ? <img src={currentUser.driverDetails.carPhotoUrl} alt="Car" className="w-full h-full object-cover" /> : <Car size={24} className="text-gray-400" />}</div>
              <div><p className="font-bold text-gray-900 dark:text-white">{currentUser.driverDetails?.carModel}</p><div className="flex items-center gap-2 text-xs text-gray-500 dark:text-gray-400 font-mono mt-0.5"><span className="bg-gray-200 dark:bg-gray-700 px-1.5 py-0.5 rounded text-gray-700 dark:text-gray-300 font-bold">{currentUser.driverDetails?.plate}</span><span className="w-3 h-3 rounded-full border border-gray-300" style={{ backgroundColor: currentUser.driverDetails?.carColor }}></span></div></div>
           </div>
           <div className="grid grid-cols-3 gap-3 mb-4">
               <button onClick={() => handleOpenEarnings('EARNINGS')} className="flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-800 p-4 rounded-2xl hover:bg-gray-100 dark:hover:bg-gray-700 transition"><div className="p-2 bg-green-100 dark:bg-green-900 rounded-full mb-1 text-green-600 dark:text-green-300"><DollarSign size={20} /></div><span className="text-xs font-bold dark:text-white">Ganancias</span></button>
               <button onClick={() => handleOpenEarnings('HOURS')} className="flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-800 p-4 rounded-2xl hover:bg-gray-100 dark:hover:bg-gray-700 transition"><div className="p-2 bg-orange-100 dark:bg-orange-900 rounded-full mb-1 text-orange-600 dark:text-orange-300"><Clock size={20} /></div><span className="text-xs font-bold dark:text-white">Horas</span></button>
               <button onClick={() => handleOpenEarnings('HISTORY')} className="flex flex-col items-center justify-center bg-gray-50 dark:bg-gray-800 p-4 rounded-2xl hover:bg-gray-100 dark:hover:bg-gray-700 transition"><div className="p-2 bg-blue-100 dark:bg-blue-900 rounded-full mb-1 text-blue-600 dark:text-blue-300"><History size={20} /></div><span className="text-xs font-bold dark:text-white">Historial</span></button>
           </div>
           {isDriverOnline && <div className="text-center p-4 bg-gray-50 dark:bg-gray-800 rounded-xl animate-pulse"><p className="text-sm font-bold text-gray-500 dark:text-gray-400">Esperando solicitudes...</p></div>}
        </div>
      )}
      ...
      {currentUser.role === UserRole.DRIVER && incomingRequest && (
         <div className="absolute inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center p-4">
            <div className="bg-white dark:bg-gray-800 w-full max-w-sm rounded-3xl shadow-2xl overflow-hidden animate-bounce-in">
               <div className="h-2 bg-gray-100 w-full"><div className="h-full bg-green-500 transition-all duration-1000 ease-linear" style={{ width: `${(incomingRequest.timeLeft / 30) * 100}%` }}></div></div>
               <div className="p-6">
                  ...
                  <div className="text-center mb-6"><h2 className="text-2xl font-black text-gray-900 dark:text-white">${incomingRequest.price.toFixed(2)}</h2><p className="text-gray-500 text-sm">{incomingRequest.distance}</p></div>
                  <div className="space-y-4 mb-6 dark:text-gray-300"><div><p className="text-xs text-gray-400">De</p><p className="font-bold">{incomingRequest.pickup}</p></div><div><p className="text-xs text-gray-400">A</p><p className="font-bold">{incomingRequest.destination}</p></div></div>
                  <div className="flex gap-3"><button onClick={() => setIncomingRequest(null)} className="flex-1 py-3 bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300 font-bold rounded-xl">Rechazar</button><button onClick={acceptRequest} className="flex-[2] py-3 bg-black dark:bg-white text-white dark:text-black font-bold rounded-xl shadow-xl">Aceptar</button></div>
               </div>
            </div>
         </div>
      )}
      ...
      {isPlacesModalOpen && <SavedPlacesModal isOpen={isPlacesModalOpen} onClose={() => {setIsPlacesModalOpen(false); setTempSavedPlaceLocation(null);}} user={currentUser} onSelect={(loc, type) => { if (type === 'PICKUP') { setPickupLocation(loc); setPickupText(loc.address || 'Ubicación guardada'); } else { setDestinationText(loc.address || 'Ubicación guardada'); setDestinationLocation(loc); } setIsPlacesModalOpen(false); }} onPickFromMap={handlePickSavedPlace} prefilledLocation={tempSavedPlaceLocation} />}
